...
every PR. It **self-heals** rather than blocks: when the derived docs are
stale, the workflow regenerates them and commits the fix back to the PR
(via `GITHUB_TOKEN`), then re-verifies in the same run. Forked PRs (where
it can't push back) get a `::warning::` advisory and the post-merge run on
`main` reconciles. **A stale derived doc never red-lights your PR**, and no
PAT or per-repo secret is required.

For the derived-file architecture to stay conflict-free across *concurrent*
PRs, the following is **recommended** (no longer required just to avoid
blocking):
...
different base commits and merge-conflict on the derived file — the
self-heal fixes each PR independently, but a rebase is still cleaner.
