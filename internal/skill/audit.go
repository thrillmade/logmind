...
func AuditSkills(repoRoot string) []AuditRow {
	...
	// os.ReadDir is already sorted, but Python's `sorted(skills_dir.iterdir())`
	// uses str-sort over Path objects which produces the same lex order
	// on POSIX systems. Be explicit so cross-platform stays predictable.
...
// readDecisionCorpus reads docs/decisions.md + docs/decisions-branches/*.md
// and returns the concatenated text. Mirrors Python's decision_text
// concatenation in audit_skills.
