...
func (s *AnthropicSuggester) Suggest(ctx context.Context, in LLMRequest) ([]Suggestion, error) {
	...
	// Deterministic order: preserve the LLM's ranking but break
	// ties via the same slug-sort the heuristic uses. (When ties
	// happen — for example when the model returns equal-rank
	// patterns — the human still gets stable output.)
...
// WriteDrafts mirrors Python's --write-drafts behaviour:
// each suggestion becomes one `skill-proposal-<slug>.md` file under
// the supplied directory. mkdir -p semantics; overwrites existing
// files (the human triggered the write, they own the directory).
