...
func splitSections(content string) []sectionInfo {
	...
	for _, m := range headerRE.FindAllStringSubmatchIndex(rest, -1) {
		...
		// whitespace. Regexp2 here already gives us the trimmed title.
...
func trimSuggestions(content string, sections []sectionInfo, total, target, budget int) []string {
	...
	for _, s := range sections {
		...
		// Python: size / total > 0.30 → integer would round down to
		// zero; Python's / on ints is float division, so compare via
		// multiplication to avoid losing precision in Go.
